window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function pushItems(state, paths, seq, source) {
      ...
      var indexByPath = {};
      for (var i = 0; i < items.length; i++) indexByPath[items[i].path] = i;
      for (var j = 0; j < paths.length; j++) {
        var path = paths[j];
        if (!isMediaPath(path)) continue;
        var next = mediaItem(path, seq, source);
        if (Object.prototype.hasOwnProperty.call(indexByPath, path)) {
          var index = indexByPath[path];
    ...
    function applyTurnEvent(state, event) {
      ...
      if (type === "tool/result") {
        ...
        if (call.name === "shengcheng" || call.name === "shengcheng_image" || call.name === "shengcheng_video" || call.name === "chrome_screenshot") {
    ...
    function itemsFromDeliverables(data) {
      var byPath = {};
      var order = [];
      function add(path, seq, source, replace) {
        if (!isMediaPath(path)) return;
        if (Object.prototype.hasOwnProperty.call(byPath, path)) {
          if (!replace) return;
        } else if (order.length >= 16) {
          return;
        } else {
          order.push(path);
        }
        byPath[path] = mediaItem(path, seq, source);
      }
      ...
      for (var i = 0; i < presented.length; i++) add(presented[i] && presented[i].path, presented[i] && presented[i].seq, "present", true);
      for (var j = 0; j < produced.length; j++) add(produced[j] && produced[j].path, produced[j] && produced[j].seq, "produced", false);
      var items = [];
      for (var k = 0; k < order.length; k++) items.push(byPath[order[k]]);
      return items;
    }
    ...
    function mergeMediaItems(primary, secondary) {
      ...
      for (var i = 0; i < all.length; i++) {
        ...
        var prev = byPath[item.path];
        if (!prev) {
          if (order.length >= 16) continue;
          order.push(item.path);
          byPath[item.path] = item;
          continue;
        }
        var prevSeq = typeof prev.seq === "number" ? prev.seq : 0;
        var nextSeq = typeof item.seq === "number" ? item.seq : 0;
        if (nextSeq >= prevSeq) byPath[item.path] = item;
      }
      var items = [];
      for (var j = 0; j < order.length; j++) items.push(byPath[order[j]]);
      return items;
    }
    ...
    function uniquePreviewItems(items) {
      ...
      for (var j = 0; j < chosen.length; j++) {
        ...
        var prev = byKey[key];
        if (!prev || newerOrSameSeq(prev, chosen[j])) byKey[key] = chosen[j];
      }
      var out = [];
      for (var k = 0; k < order.length && out.length < 16; k++) out.push(byKey[order[k]]);
      return out;
    }
    ...
    function fileUrl(path, kind, rev) {
      ...
      var url = api + "?path=" + encodeURIComponent(path);
      if (rev == null || String(rev) === "") return url;
      return url + "&rev=" + encodeURIComponent(String(rev));
    }
    ...
    function Gallery(props) {
      ...
      var rawItems = selectZhanshi(props) || [];
      ...
      return React.createElement(
        ...
        resolved.map(function (item) {
          var url = fileUrl(item.path, item.kind, item.seq);
          var open = function () {
          ...
          return React.createElement(
            "figure",
            { key: item.path + ":" + String(item.seq == null ? "" : item.seq), className: "zs-item" },
            media,
            React.createElement(
              "figcaption",
